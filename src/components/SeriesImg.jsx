  ...
  if (err) return (
    <div className={className} style={{...style, background:c, display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden', position:'relative'}}>
      <svg width="55%" viewBox="0 0 80 80" style={{display:'block'}}>
        ...
        <rect x="26" y="61" width="28" height="2" rx="1" fill="rgba(30,42,32,0.14)"/>
      </svg>
    </div>
  );
  return (
    <div className={className} style={{...style, position:'relative', overflow:'hidden'}}>
      ...
      <div style={{position:'absolute',inset:0,background:'linear-gradient(to bottom,transparent 50%,rgba(30,42,32,0.35))',pointerEvents:'none'}}/>
    </div>
  );
}
